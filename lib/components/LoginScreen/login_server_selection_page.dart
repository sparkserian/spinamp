...
import 'package:finamp/config/app_config.dart';
...
class _LoginServerSelectionPageState extends State<LoginServerSelectionPage> {
  ...
  bool _hasAttemptedConfiguredServer = false;

  Future<void> _attemptConfiguredServer() async {
    if (_hasAttemptedConfiguredServer) {
      return;
    }

    final attemptedUrl = widget.serverState.baseUrl;
    if (attemptedUrl == null || attemptedUrl.isEmpty) {
      return;
    }

    _hasAttemptedConfiguredServer = true;
    widget.serverState.baseUrlToTest = attemptedUrl;
    widget.serverState.updateCallback?.call();

    try {
      await widget.serverState.testServerConnection(attemptedUrl);
      if (!mounted || widget.serverState.baseUrl != attemptedUrl) {
        return;
      }
      ...
      _loginServerSelectionPageLogger
          .finer("Configured server auto-connect failed: $error");
    } finally {
      if (mounted && widget.serverState.baseUrlToTest == attemptedUrl) {
  ...
  void initState() {
    ...
    widget.serverState.baseUrl ??= kBakedJellyfinUrl;
    ...
    if ((widget.serverState.baseUrl?.isNotEmpty ?? false) &&
        widget.serverState.manualServer == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) {
          return;
        }
        _attemptConfiguredServer();
      });
    }
  }
      ...
      child: AutofillGroup(
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              initialValue: widget.serverState.baseUrl ?? kBakedJellyfinUrl,
              autocorrect: false,
