...
void main() async {
  ...
  try {
    ...
    _migrateSortOptions();
    await _migrateDesktopDownloadLocations();
...
Future<void> _migrateDesktopDownloadLocations() async {
  if (!(Platform.isMacOS || Platform.isWindows || Platform.isLinux)) {
    return;
  }

  final finampSettings = FinampSettingsHelper.finampSettings;
  final hasExistingDownloads = GetIt.instance<Isar>()
          .downloadItems
          .where()
          .filter()
          .not()
  ...
  DownloadLocation? internalSupportLocation;
  DownloadLocation? cacheLocation;

  for (final location in finampSettings.downloadLocationsMap.values) {
    if (location.baseDirectory == DownloadLocationType.internalSupport) {
      internalSupportLocation = location;
    } else if (location.baseDirectory == DownloadLocationType.cache) {
  ...
  if (internalSupportLocation == null &&
      cacheLocation != null &&
      !hasExistingDownloads) {
    cacheLocation.baseDirectory = DownloadLocationType.internalSupport;
    cacheLocation.relativePath = null;
    await cacheLocation.updateCurrentPath();
    internalSupportLocation = cacheLocation;
    changed = true;
  }

  if (internalSupportLocation == null && cacheLocation == null) {
    internalSupportLocation = await DownloadLocation.create(
      name: "Internal Storage",
      baseDirectory: DownloadLocationType.internalSupport,
    );
    finampSettings.downloadLocationsMap[internalSupportLocation.id] =
        internalSupportLocation;
    changed = true;
  }

  final resolvedDefaultLocationId =
      internalSupportLocation?.id ?? cacheLocation?.id;

  if (resolvedDefaultLocationId != null &&
      (finampSettings.defaultDownloadLocation == null ||
          !finampSettings.downloadLocationsMap
              .containsKey(finampSettings.defaultDownloadLocation))) {
    finampSettings.defaultDownloadLocation = resolvedDefaultLocationId;
    changed = true;
  }
...
class _FinampState extends ConsumerState<Finamp> with WindowListener {
  ...
  Widget build(BuildContext context) {
    final shouldDismissFocusOnBackgroundTap =
        Platform.isAndroid || Platform.isIOS;

    Widget buildMaterialAppShell() {
    ...
    Widget shell = buildMaterialAppShell();

    if (shouldDismissFocusOnBackgroundTap) {
